...
def inference_top_down_pose_model(model,
                                        ...
                                        return_heatmap=False,
                                        outputs=None):
    """Inference a batch of images with lists of person bounding boxes.

    Args:
        model (nn.Module): The loaded pose model.
        img_or_paths (list[str|np.ndarray]): List of image filenames or loaded images.
        all_person_results (list[list[dict]], optional): Lists of detected persons for each image.
        bbox_thr (float | None): Threshold for bounding boxes.
        format (str): bbox format ('xyxy' | 'xywh'). Default: 'xywh'.
        dataset (str): Dataset name.
        dataset_info (DatasetInfo): Class containing all dataset info.
        return_heatmap (bool): Flag to return heatmap.
        outputs (list[str] | tuple[str]): Names of layers whose outputs need to be returned.

    Returns:
        list[tuple]: List of results for each image.
    """
    # get dataset info
    ...
    all_pose_results = []
    all_returned_outputs = []

    for img_or_path, person_results in zip(img_or_paths, all_person_results):
        if len(person_results) == 0:
            all_pose_results.append([])
            all_returned_outputs.append([])
            continue

        bboxes = np.array([box['bbox'] for box in person_results])

        #print(f"Bounding boxes shape: {bboxes.shape}")
        #print(f"Bounding boxes content: {bboxes}")
        ...
        with OutputHook(model, outputs=outputs, as_tensor=False) as h:
            ...
            returned_outputs = h.layer_outputs
            all_returned_outputs.append(returned_outputs)
    ...
    return all_pose_results, all_returned_outputs
...
def process_mmdet_results(mmdet_results, cat_id=1):
    """Process mmdet results for a batch, and return a list of bboxes.

    Args:
        mmdet_results (list[list|tuple]): List of mmdet results for each image.
        cat_id (int): category id (default: 1 for human)

    Returns:
        person_results (list[list]): A list of lists of detected bounding boxes for each image.
    """
    all_person_results = []

    for idx, results in enumerate(mmdet_results):
        ...
        bboxes = det_results
        #print(f"Image {idx}: Detected bboxes shape: {bboxes.shape}")
